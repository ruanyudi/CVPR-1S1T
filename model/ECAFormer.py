...
class DMSABlock(nn.Module):
    ...
    def forward(self, x, y):
        x = x.permute(0, 2, 3, 1)
        y = y.permute(0, 2, 3, 1)
        for (attn, ff_x,ff_y) in self.blocks:
...
class CrossAttenUnet(nn.Module):
    def __init__(self, in_dim=3, out_dim=3, dim=31, level=2, num_blocks=[2, 4, 4]):
        ...
        self.dim = dim
        self.level = level
        self.embedding_x = nn.Conv2d(in_dim, self.dim, 3, 1, 1, bias=False)
        self.embedding_y = nn.Conv2d(in_dim, self.dim, 3, 1, 1, bias=False)
        ...
        self.mapping = nn.Sequential(
            ...
            nn.Conv2d(self.dim * 2, self.dim*2, 3, 1, 1, bias=True),
            nn.LeakyReLU(negative_slope=0.01, inplace=True),
            nn.Conv2d(self.dim * 2, out_dim, 3, 1, 1, bias=True),
            nn.LeakyReLU(negative_slope=0.01, inplace=True)
        )
    ...
    def forward(self, x, y):
        ...
        out = self.mapping(torch.cat([x, y], dim=1))
        return out
...
class ECAFormer_onestage(nn.Module):
    ...
    def forward(self, img):
        instance, modality = self.RepNet(img)
        output_img = self.CrossAttUnet(instance,modality)
        return output_img,instance,modality
...
class ECAFormer(nn.Module):
    def __init__(self,in_channels=91, out_channels=91, n_feat=40, level=2, num_blocks=[1, 2, 2],stage=1):
        ...
        for i in range(stage):
            modules.append(ECAFormer_onestage(in_channels, out_channels, n_feat, level,num_blocks))
        self.body = nn.Sequential(*modules)
    def forward(self, img):
        for net in self.body:
            img,instance,modality = net(img)
        return img,instance,modality
